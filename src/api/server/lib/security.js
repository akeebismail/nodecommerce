...
const SET_TOKEN_AS_REVOKEN_ON_EXEPTION = true;
...
const checkUserScope = (requiredScope, req, res, next)=> {
    if (DEVELOPER_MODE  === true){
        ...
        req.user && req.user.scopes && req.user.scopes.length > 0
...
const verifyToken = token => {
    return new Promise((resolve, reject)=>{
        jwt.verfiy(token, settings.jwtSecretKey, (error, decoded)=>{
            if (error){
                reject(error)
            } else {
                resolve(decoded)
...
const checkTokenInBlacklistCallback = async (req, payload, done) => {
    try {
        ...
        done(e, SET_TOKEN_AS_REVOKEN_ON_EXEPTION);
    }
};
